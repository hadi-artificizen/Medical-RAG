...
def register(req: RegisterRequest):
    ...
    cur.execute("INSERT INTO users (email, password) VALUES (%s, %s)", ( req.email, req.password))
    conn.commit()
...
def login(req: LoginRequest):
    cur.execute("SELECT * FROM users WHERE email=%s AND password=%s", (req.email, req.password))
    user = cur.fetchone()
    if user:
        return {"message": "Login successful", "user": {"email": req.email}}
    else:
        return {"error": "Invalid credentials"}

@app.get("/chatrooms/{email}")
def get_chatrooms(email: str):
...
def chat(req: ChatRequest):
    ...
    cur.execute("INSERT INTO chat_threads (chatroom_id, user_message) VALUES (%s, %s) RETURNING id",
                (req.chatroom_id, req.message))
    ...
    def generate():
        ...
        # If prescription detected → send email to user from DB
...
async def query_documents(req: QueryRequest):
    ...
    # Save query & response in DB under a default chatroom (if needed)
    ...
    # Send email if prescription found
